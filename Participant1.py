...
class ParticipantProcess:
    def __init__(self):
        try:
            self.participantid="Participant_A"
            self.soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.soc.connect((socket.gethostname(), 7891)) 
            self.soc.sendall("Participant_A".encode())
        ...
        while (True):
            try:
                                ...
                                def reConnectCoordinator():
                                    while True:
                                        try:
                                            ...
                                            print(f"<<{self.participantid}>> Reonnected to Coordinate Server")
                                            self.soc.sendall("Participant_A".encode())
                ...
                if msgFromCoordinator == "Ready For Poling" and msgAfterTimeOut == "rollback":
                    ...
                    print("Participant_A   >>> Response Fail Due to Coordinator TimeOut")
                    msgFromCoordinator=""
                    msgAfterTimeOut = ""                      
                    ...
                    if (randval %7 != 0):
                        ...
                        print("Participant_A   >>>   response Ready") 
                    else :
                        self.soc.sendall("Fail".encode()) 
                        print("Participant_A   >>>   Response Fail") 
                    ...
                    msgFromCoordinator="" 
                        ...
                        print("Participant_A   >>>   Response Task Completed and lock rollbacked") 
                        msgFromCoordinator=""
                elif (msgFromCoordinator=="rollback") :
                        self.soc.sendall("Task aborted and lock rollbacked".encode()) 
                        print("Participant_A   >>>   Response Task aborted and lock rollbacked") 
                        msgFromCoordinator=""
            except Exception as e:
                print("Participant_A   >>>   Response Task aborted and lock rollbacked") 
                break 
