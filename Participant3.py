...
class ParticipantProcess:
    def __init__(self):
        try:
            self.participantid="Participant_C"
            self.soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.soc.connect((socket.gethostname(), 7891)) 
            self.soc.sendall("Participant_C".encode()) 
        ...
        while (True):
            try:
                ...
                if msgFromCoordinator == "Ready For Poling" and msgAfterTimeOut == "rollback":
                    ...
                    print("Participant_C    >>> Response Fail Due to Coordinator TimeOut")
                    msgFromCoordinator=""
                    msgAfterTimeOut = ""                      
                    ...
                    if (randval %7 != 0):
                        ...
                        print("Participant_C    >>>   response Ready") 
                    else :
                        self.soc.sendall("Fail".encode()) 
                        print("Participant_C    >>>   Response Fail") 
                    ...
                    msgFromCoordinator="" 
                        ...
                        print("Participant_C    >>>   Response Task Completed and lock rollbacked") 
                        msgFromCoordinator=""
                elif (msgFromCoordinator=="rollback") :
                        self.soc.sendall("Task aborted and lock rollbacked".encode()) 
                        print("Participant_C    >>>   Response Task aborted and lock rollbacked") 
                        msgFromCoordinator=""
            except Exception as e:
